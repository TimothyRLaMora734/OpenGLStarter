    ...
    void putByte(uint8_t byte) {
        ...
        if (byte == 0x00 && nalState == None)
            ...
            if (writingState == None){
                ...
                buffer.clear();
                if (emulationState == EMULATION_FORCE_SKIP){
                    fprintf(stdout, "H264 NAL emulation prevention pattern write error\n");
                    exit(-1);
                }
                emulationState = None;
                return;
        ...
        if (writingState == Data){
            ...
            /*
            //
            // Force Remove the Emulation Prevention Bytes
            //
...
public:
    ...
    void chunkDetectedH264(const uint8_t* ibuffer, int size) {
        ...
        //else if (nal_bit == (NAL_IDC_PICTURE | NAL_TYPE_PPS) ) {
            ...
            mFLVWritter.writeUInt8(0xFC | 0x03); //reserved (6 bits), NULA length size - 1 (2 bits)
