...
public:
        ...
        for(int z=0;z<zDiv;z++){
            for(int x=0;x<xDiv;x++){
                ...
                if (z<zDiv-1 && x<xDiv-1){
                    indicesBuffer[count + 0] = x + z * xDiv;//0
                    indicesBuffer[count + 1] = x + (z+1) * xDiv;//1
                    indicesBuffer[count + 2] = x + 1 + (z+1) * xDiv;//2

                    indicesBuffer[count + 3] = x + z * xDiv;//0
                    indicesBuffer[count + 4] = x + 1 + (z+1) * xDiv;//2
                    indicesBuffer[count + 5] = x + 1 + z * xDiv;//3
    ...
    void draw(const mat4 &transform){
        app->drawPrimitive6(GL_TRIANGLES, transform, vertexBuffer, colorBuffer, indicesBuffer,  indiceCount );
    }
